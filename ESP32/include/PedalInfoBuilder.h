...
class PedalInfoBuilder
{
    ...
    void BuildInfoString(uint8_t pedaltype,const char* board, float loadcellShift, float loadcellVariance, float psuVoltage,long maxEndStop, uint32_t currentPosition);
    void LogClear();
    void BuildESPNOWInfo(uint8_t pedalType, int32_t* rssi);
    void BuildRandonString(const char* string);
};
PedalInfoBuilder pedalInfoBuilder;

void PedalInfoBuilder::BuildInfoString(uint8_t pedalType,const char* board, float loadcellShift, float loadcellVariance, float psuVoltage,long maxEndStop, uint32_t currentPosition)
{
...
void PedalInfoBuilder::BuildRandonString(const char* string)
{
    memcpy(logESPNOWString, string, strlen(string));
}
